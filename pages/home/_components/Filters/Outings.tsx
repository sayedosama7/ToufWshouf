...
import { useGetNationalityQuery } from '@/store/Filter/FetchNationalityApi';
...
const Outings: FunctionComponent<Props> = () => {
    ...
    const { data: nationalities } = useGetNationalityQuery({});
    const nationalityData = nationalities?.items || [];
    ...
    return (
        <Grid container justifyContent="center" alignItems="end">
            ...
            <Grid item xs={2}>
                <Autocomplete
                    id="country"
                    options={nationalityData.map((n: Nationality) => ({ label: n.country_name }))}
                    autoHighlight
                    sx={{
                        backgroundColor: 'body.light',
                        borderRadius: '0',
